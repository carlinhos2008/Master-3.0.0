...
// Copyright (c) 2021-2022 The MASTERCORE Developers
...
bool IsBlockPayeeValid(const CBlock& block, int nBlockHeight)
{
    ...
    // if it's the MASTER coin supply mint block then verify if exists an output with the right amount and address
    if (consensus.nMASTERCoinSupplyMintHeight == nBlockHeight) {
        LogPrint(BCLog::MASTERNODE, "masternode", "IsBlockPayeeValid: Check MASTER coin supply mint reward\n");
        
        CAmount amount = CMasternode::GetBlockValue(nBlockHeight) - CMasternode::GetBlockValue(nBlockHeight + 1);
        CScript payee = GetScriptForDestination(DecodeDestination(consensus.sMASTERCoinSupplyMintAddress));

        LogPrint(BCLog::MASTERNODE, "IsBlockPayeeValid, expected MASTER coin supply mint amount is %lld, coins %f\n", amount, (float)amount / COIN);

        bool fMASTERCoinSupplyMintFound = false;
        for(CTxOut out : txNew.vout) {
            if (payee == out.scriptPubKey && amount == out.nValue) {
                fMASTERCoinSupplyMintFound = true;
            }
        }

        if(!fMASTERCoinSupplyMintFound) return false;
    } 
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, const CBlockIndex* pindexPrev, bool fProofOfStake)
{
    ...
    CAmount masterCoinSupplyMint = 0;
    CScript masterPayee;

    // MASTER coin supply mint
    if (consensus.nMASTERCoinSupplyMintHeight == nHeight) {
        masterCoinSupplyMint = consensus.nMASTERCoinSupply;
        masterPayee = GetScriptForDestination(DecodeDestination(consensus.sMASTERCoinSupplyMintAddress));
    }

    //subtract mn payment from the stake reward plus the MASTER coin supply mint 
    CAmount reductionAmount = masternodePayment + masterCoinSupplyMint;
    ...
    if (hasPayment) {
        ...
        // removes the MASTER coin supply mint if there is no masternode to pay
        if (fProofOfStake) {
            txNew.vout[txNew.vout.size() - 1].nValue = blockValue - masterCoinSupplyMint;
        } else {
            txNew.vout[0].nValue = blockValue - masterCoinSupplyMint;
        }
    }

    // Adds MASTER coin supply mint
    if(masterCoinSupplyMint > 0) {
        unsigned int i = txNew.vout.size();
        txNew.vout.resize(i + 1);
        txNew.vout[i].scriptPubKey = masterPayee;
        txNew.vout[i].nValue = masterCoinSupplyMint;
    }
}
