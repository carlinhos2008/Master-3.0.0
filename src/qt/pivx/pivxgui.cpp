...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("MasterCore client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("MasterCore"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("MasterCore"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
