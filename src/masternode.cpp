...
{
    if (nHeight >  2160000) return 500000 * COIN;
    if (nHeight >  1728000) return 350000 * COIN;
    if (nHeight >  1296000) return 300000 * COIN;
    if (nHeight >   864000) return 250000 * COIN;
    if (nHeight >   432000) return 200000 * COIN;
...
CAmount CMasternode::GetBlockValue(int nHeight)
{
    ...
    // MASTER coin supply for distribution
    if (nHeight == consensus.nMASTERCoinSupplyMintHeight) return consensus.nMASTERCoinSupply + GetBlockValue(nHeight + 1);

    if (nHeight >  1200000) return       10 * COIN;
    if (nHeight >   800000) return       15 * COIN;
    if (nHeight >    47500) return       10 * COIN;
    if (nHeight >    45000) return       50 * COIN;
    if (nHeight >    20000) return       10 * COIN;
    if (nHeight >        1) return        1 * COIN;
    if (nHeight >        0) return  8800000 * COIN; 
...
CAmount CMasternode::GetMasternodePayment(int nHeight)
{
    ...
    // MASTER coin supply for distribution
    if (nHeight == consensus.nMASTERCoinSupplyMintHeight) return GetMasternodePayment(nHeight + 1);

    if (nHeight > 864000) return GetBlockValue(nHeight) * 75 / 100;
    if (nHeight >  47500) return GetBlockValue(nHeight) * 80 / 100;
    if (nHeight >  45000) return GetBlockValue(nHeight) * 95 / 100;
    if (nHeight >    250) return GetBlockValue(nHeight) * 80 / 100;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 MASTER tx got MASTERNODE_MIN_CONFIRMATIONS
